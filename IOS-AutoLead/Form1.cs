...
namespace IOS_AutoLead
{
    public partial class Form1 : Form
    {
            ...
            for(int i=0;i<  dataGridView1.Rows.Count-1;i++)
            {
                ...
                if (dataGridView1.Rows[i].Cells[0].Value.ToString().Trim().ToLower() == "true")
                {
                    ...
                    foreach (string str in runthr.GetAppList())
                    {
                        ...
                        {
                            if (arrnameapp[1].Trim().Equals(dataGridView1.Rows[i].Cells[3].Value.ToString().Trim())){
                    ...
                    {
                        if (r.applist.Trim().Equals(nameapp))
                        {
                            r.totalrun += 1;
                            c = 1;
                        }
                    ...
                    {
                        string[] arr = null;
                        if (File.Exists(iStatic.diraidIphone + "/Scirpt.txt"))
                        {
                            arr = File.ReadAllLines(iStatic.diraidIphone + "/Scirpt.txt");
                        }
                        ThreadRunLead(off,checkBox2.Checked,arr,int.Parse(txtTimeLoadURL.Text));
        ...
        public void ThreadRunLead(Offer offer, bool blWipeFull, string[] arrScript, int timeoutUrl)
        {
            ThreadRunLead runthr = new ThreadRunLead();
            ...
            if (runthr.Wipe(offer.NameApp, blWipeFull))
            {
                ...
                if (iDevice.Country != null)
                {
                    ...
                    if (runthr.openUrl(offer.Url, timeoutUrl))
                    {
                        ...
                        foreach (Run r in lrun)
                        {
                            if (r.applist.Trim().Equals(offer.NameApp))
                            {
                               r.totalInstall += 1;
        ...
        private void comboBox3_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            foreach (Backup blup in lbackup)
            {
                ...
                if (ck == 0 && blup.appList.Equals(cbxRRSbyApp.Text))
                {
                ...
                if (cbRRSbyDay.Checked)
                {
                    if (blup.appList.Equals(cbxRRSbyApp.Text) && blup.timecreate.Equals(cbxRRSbyDay.Text))
                    {
                ...
                {
                    if (blup.appList.Equals(cbxRRSbyApp.Text))
                    {
        ...
        private void comboBox2_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            foreach (Backup blup in lbackup)
            {
                ...
                {
                    if (blup.timecreate.Equals(cbxRRSbyDay.Text))
                    {
                        dataGridView3.Rows.Add(blup.filename, blup.appList, blup.total, blup.country, blup.timecreate);
                    }
                }
            ...
            if (dataGridView3.SelectedRows.Count == 1)
            {
                ...
                new Thread(() =>
                {
                    base.Invoke(new MethodInvoker(delegate
                    {
                        ...
                        runthr.Restore(dataGridView3.Rows[irow].Cells[0].Value.ToString(), dataGridView3.Rows[irow].Cells[1].Value.ToString());
                    }));
                }).Start();
            }     
        }
